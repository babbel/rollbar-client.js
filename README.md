...
| `transform`        | N        | `Function` | `no-op function`                                                        | Enables users to transform the payload that will be sent to Rollbar. The function must mutate the passed payload. Function signature: `transform(payload, configuration)` where `payload` is the payload object about to be sent to Rollbar and `configuration` is the configuration object                                                                                                                |
